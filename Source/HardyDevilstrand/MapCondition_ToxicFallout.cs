...
namespace HardyDevilstrand
{
	public class MapCondition_ToxicFallout : MapCondition
	{
		...
		public MapCondition_ToxicFallout() : base()
		{
		...
		public override void DoCellSteadyEffects(IntVec3 c)
		{
			if (!c.Roofed())
			{
				...
				for (int i = 0; i < thingList.Count; i++)
				{
					...
					if ((thing is Plant) && !(thing.def.defName == "PlantDevilstrand"))
					{
		...
		{
			return MapConditionUtility.LerpInOutValue((float)base.TicksPassed, (float)base.TicksLeft, LerpTicks, 0.5f);
		}
